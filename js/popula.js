...
const firstpage = async () => {
    const res = await fetch(
        `https://api.themoviedb.org/3/discover/movie?api_key=6b2dec73b6697866a50cdaef60ccffcb&sort_by=popularity.desc&include_adult=false&page=${intialPage}&with_genres=${categoryId}&certification_country=US&certification.lte=G`
    );
    const data = await res.json();
...
const airingTodayfun = async () => {
    ...
    const data = await res.json();
    const maxPages = 25; // Set the maximum number of pages
    let totalPages = Math.min(data.total_pages, maxPages);
    return totalPages;
};
